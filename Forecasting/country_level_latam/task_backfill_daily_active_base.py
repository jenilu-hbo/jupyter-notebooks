...
import sys
from utils import *
from queries import back_fill_base_query
...
#         max_end_date = max_end_date.max_date.values[0]
        max_end_date = pd.to_datetime('2021-09-24')
        max_date = pd.to_datetime('2021-10-18')
        ...
        while (t.strftime('%Y-%m-%d')>=max_end_date.strftime('%Y-%m-%d')) and (t.strftime('%Y-%m-%d')<max_date.strftime('%Y-%m-%d')):
            print ('start_date:' + t.strftime('%Y-%m-%d'))
            if t.strftime('%Y-%m-%d') >= '2021-06-29':
                ct = self.run_query('''
                    SELECT count(*) as ct 
                    FROM max_dev.workspace.{table}
