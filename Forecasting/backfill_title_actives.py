...
today = date.today()
today = (today - timedelta(days=2)).strftime("%d/%m/%Y")

max_date = run_query('''SELECT MAX(real_date) as max_date FROM max_dev.workspace.{table}
                    '''.format(
                        table = table_name
                        ))
...
while (t>=date(2020,5,23) and t<=today:
    print (t)
