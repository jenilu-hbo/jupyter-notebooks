...
df_60_00 = spark.sql('''
                     ...
                     , CASE WHEN r.recency_window_end >= '2023-12-01' 
                            THEN 'current'
                            ELSE 'library'
                            END AS old_new
...
df_final[df_final['frac'] == 1]['user_id'].nunique()/df_final['user_id'].nunique() # 83% users watched mixed content
...
df_library = df_1[(df_1['old_new']=='library') & (df_1['monthly_hours_viewed']>=0.5)] #80% of the population
...
for m in df_60.tenure_bucket.unique():
    df_seg_amw= df_60[df_60['tenure_bucket'] == m]
    # add seg total
    df_60_t = df_seg_amw.groupby(by=['user_id','is_cancel','sub_month'])[['monthly_title_viewed', 'monthly_hours_viewed']].sum().reset_index()
    df_60_s = get_churn_bin(df_60_t, [])
    df_60_s['title_viewed_bin'] = df_60_s['title_viewed_bin'].astype(float)
    ...
    for i in df_60[groupby_col].unique():
        ...
        df_seg_amw= df_60[(df_60['tenure_bucket'] == m)]# & (df_60_00[groupby_col] == i)]
